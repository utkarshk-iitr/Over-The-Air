...
class MerkleTree:
    ...
    def getAuthenticationPath(self, value, i_val):
        ...
        def findNode(node, depth, leaf_index):
            ...
            if node.left is None and node.right is None:
                if leaf_index==i_val:
                    if i_val%2==0: path[str(depth)+"l"] = node.value
                    else: path[str(depth)+"r"] = node.value
                    return True
                else: return False
    ...
    def getAncestorslist(self,value):
        ...
        def findNode(node,value):
            if node is None: return False
            elif node.value == value: return True
            else:
...
def Ver_merkle_path(auth_path,root_hash):
    ...
    for key, value in auth_path.items():
        if skip_count >= 3:
            if key[1]=="l":
                prev_hash = hashlib.sha256(auth_path[key].encode()+prev_hash.encode()).hexdigest()
            elif key[1]=="r":
                prev_hash = hashlib.sha256(prev_hash.encode()+auth_path[key].encode()).hexdigest()
            
        else:
            if key[1]=="l": value1 = auth_path[key]
            elif key[1]=="r": value2 = auth_path[key]
            skip_count += 1

            if skip_count==2:
                prev_hash = hashlib.sha256(value1.encode()+value2.encode()).hexdigest()
                skip_count += 1

    if prev_hash==root_hash: return 1
    else: return 0

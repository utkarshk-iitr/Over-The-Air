...
Connects to DH server for secure communication using DH key exchange + AES-GCM
...
class DHClient:
    ...
    def connect_and_handshake(self):
        ...
        try:
            ...
            # Receive server's DH parameters and public key
            try:
                params_len = PQCrypto.read_u32_be(SecureFrame.recv_all(self.sock, 4))
                server_params_bytes = SecureFrame.recv_all(self.sock, params_len)
                
                server_pub_len = PQCrypto.read_u32_be(SecureFrame.recv_all(self.sock, 4))
                server_pub_bytes = SecureFrame.recv_all(self.sock, server_pub_len)
            except Exception as e:
                print(f"[client] Failed to receive server DH data: {e}")
                return False

            # Generate client DH keypair using server's parameters
            from cryptography.hazmat.primitives import serialization
            from cryptography.hazmat.backends import default_backend
            
            try:
                # Load server's DH parameters
                server_params = serialization.load_pem_parameters(server_params_bytes, backend=default_backend())
                
                # Generate client private key using server's parameters
                client_private_key = server_params.generate_private_key()
                ...
                self.sock.send(PQCrypto.write_u32_be(len(client_public_key_bytes)))
                SecureFrame.send_all(self.sock, client_public_key_bytes)
                ...
                # Derive AES key from shared secret (use same salt as server)
                salt = b"server-salt-v1"  # Match server salt for consistency
                ...
                print(f"[client] DH handshake completed in {handshake_time:.4f} seconds")
                li.append(handshake_time)
                return True
                
            except Exception as e:
                print(f"[client] DH key exchange failed: {e}")
                import traceback
                traceback.print_exc()
                return False
    ...
    def download_updates(self):
        ...
        try:
            ...
            print(f"File downloaded successfully in {transfer_time:.4f} seconds")
            print(f"Total decryption time: {decrypt_time:.4f} seconds")
            
            # Log: Handshake, Transfer Time, Decrypt Time (no ZKP time)
            li.append(transfer_time)
            li.append(decrypt_time)
            fw.writerow(li)
            li.clear()
...
if __name__ == "__main__":
    ...
    client = DHClient(sys.argv[1], int(sys.argv[2]))
    client.run()
    f2.close()

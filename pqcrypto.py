...
class Certificate:
    def create_cert(kyber_pub,dilithium_pub,sig_manager):
        message = kyber_pub + dilithium_pub
        signature = sig_manager.sign(message)
        
        # Certificate: [kyber_pub_len][kyber_pub][dilithium_pub_len][dilithium_pub][sig_len][signature]
        cert = b''
        cert += PQCrypto.write_u32_be(len(kyber_pub))
        cert += kyber_pub
        cert += PQCrypto.write_u32_be(len(dilithium_pub))
        cert += dilithium_pub
    ...
    def parse_cert(cert):
        ...
        kyber_len = PQCrypto.read_u32_be(cert[pos:pos+4])
        pos += 4
        kyber_pub = cert[pos:pos+kyber_len]
        pos += kyber_len
        
        dilithium_len = PQCrypto.read_u32_be(cert[pos:pos+4])
        pos += 4
        dilithium_pub = cert[pos:pos+dilithium_len]
        pos += dilithium_len
        ...
        return kyber_pub, dilithium_pub, signature
